...
void setup(){
 ...
 asteroidPics[0] = loadImage("asteroide1.jpg");
 asteroidPics[1] = loadImage("asteroide2.jpg");
 ...
 if( lives >= 0 && asteroids.size()>0){
   ...
   if(darkCounter < darkCounterLimit){
    ...
    rect(0,0,width,height);
     ...
     textFont(font, 16);
     fill(0, 200);
     text(" CLICK PARA EMPEZAR UN NUEVO JUEGO", width/2-80-2, height*.9-1);
     textFont(font, 15);
     fill(255);
     text("CLICK PARA UN EMPEZAR NUEVO JUEGO", width/2-80, height*.9);
   }
 } else {
     background(0);
     ship = new Ship();
     ...
     fill(255);
     if(stage > -1){
         text("Mision " + (stage + 1) + " Finalizada", width/2-120, height/2);
     } else {
         text("PILOTO DE COMBATE", width/2-80, height/2);
     }

     textFont(font, 15);
     fill(255);
     text("PRECIONA CLICK PARA EMPEZAR " + (stage + 2), width/2-100, height*.75);
...
class Bullet{
 ...
 int timeOut = 24 * 2;
 float alpha;
 PImage img = loadImage("lacer.png");
